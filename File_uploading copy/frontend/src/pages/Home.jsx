// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< File Upload on cloudinary directly without any server  ----->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
//     // formData.append("cloud_name", "dgj0o5wfe");  // cloudname append krna optional h vo already api m hota h
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<< File Upload on first on server then cloudinary ----->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
const Home=()=>{
 ...
 try {
const res = await axios.post("http://localhost:8000/upload-multiple",
  formData,
